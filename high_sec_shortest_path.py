...
try:
    with open("route_maps/high_sec_route_map.json", "r") as f:
...
def construct_path(n, parent_map, save_path=False):
    ...
    if save_path:
        if path[0] in CACHED_ROUTE_MAP:
            ...
            CACHED_ROUTE_MAP[path[0]] = {path[-1] : path}
        target_name = ("route_maps/high_sec_route_map_%0.4f.json" % (time.time()))
        try:
            shutil.move("route_maps/high_sec_route_map.json", target_name)
        except:
            pass
        with open("route_maps/high_sec_route_map.json", "w") as f:
